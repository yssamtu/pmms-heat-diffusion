...
inline const double *get_lower_row_index(const double arr[],
                                         const struct parameters *p, size_t i)
{
    return !i ? p->tinit : arr + (i - 1) * p->M;
}

inline const double *get_upper_row_index(const double arr[],
                                         const struct parameters *p, size_t i)
{
    return i == p->N - 1 ? p->tinit + i * p->M : arr + (i + 1) * p->M;
}

double get_sum_neighbours(const double arr[], const struct parameters *p,
                          size_t i, size_t j, double w_direct, double w_diagonal)
{
...
bool caculate_metrix_with_st(const double init[], double avg[],
                             const struct parameters *p, struct results *r,
                             const double w_direct, const double w_diagonal)
{
    bool stop = true;
    size_t num = p->N * p->M;
...
bool caculate_metrix(const double init[], double avg[],
                     const struct parameters *p, struct results *r,
                     const double w_direct, const double w_diagonal)
{
    bool stop = true;
